  ...
  stages {
   stage ('Initialize') {
    ...
    stage('Push Artifact to S3') {
      steps {
        sh 'aws s3 cp webapp/target/webapp.war s3://bulansi'
      }
    }
    
    stage('Deploy to tomcat') {
      steps {
        sh 'sudo scp -i $tomcat_pem -o "StrictHostKeyChecking=no" webapp/target/webapp.war ubuntu@18.216.147.28:/opt/tomcat/webapps'
      }
    }
